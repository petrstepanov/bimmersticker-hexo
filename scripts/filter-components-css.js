...
hexo.extend.filter.register('after_render:html', (html) => {
  ...
  for(componentClass of componentClasses){
    ...
    if (fs.existsSync(path)){
      css += fs.readFileSync(path);
      hexo.log.info("Component \"" + componentClass + "\" CSS loaded");
    } else {
      hexo.log.error("Component \"" + componentClass + "\" CSS not found!");
    }

    var pathDev = "./themes/blueberry/source/css/components/" + componentClass + "-dev.css";
    if (fs.existsSync(pathDev)){
      cssDev += fs.readFileSync(pathDev);
      hexo.log.info("Component \"" + componentClass + "\" development CSS loaded");
    } else {
      hexo.log.error("Component \"" + componentClass + "\" development CSS not found!");
    }
  }
